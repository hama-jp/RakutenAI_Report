...
### 4.4 LoRA相当のコストで「独自モデル」を装う構造

ここで注目すべきは、Rakutenが採った手法の**対外的な見え方**です。

通常、既存モデルをLoRAでファインチューニングする場合、成果物は小さなLoRAアダプタファイル（数百MB程度）として配布されます。利用者はベースモデル（この場合DeepSeek-V3）を別途ダウンロードし、アダプタを適用する必要があります。この配布形態では、**ベースモデルへの依存関係は一目瞭然**であり、ライセンス上の関係性も明確です。

```
【通常のLoRAファインチューニングの場合】
  成果物: LoRAアダプタファイル（数百MB）
  配布時: "base_model: deepseek-ai/DeepSeek-V3" と明記が必要
  見え方: 明らかにDeepSeek-V3の派生物
  コスト: 低ランク層のみの学習 → 低コスト

【Rakutenが実際に行ったこと】
  手法:   MLA内蔵の低ランク射影層（LoRAと同等の数学的構造）だけをファインチューニング
  コスト: LoRAと実質同等 → 低コスト
  配布時: 671Bパラメータのフルモデルとして配布
  見え方: 独立した「RakutenAI-3.0」という独自モデル
```

DeepSeek-V3のMLAアーキテクチャには、LoRAと数学的に同等の低ランク行列分解（A行列×B行列）が**最初から組み込まれています**。Rakutenはこの構造を利用し、**LoRAと実質同じ計算コスト**で低ランク層のみをファインチューニングしました。しかし、その結果をLoRAアダプタとしてではなく、**全重みを含むフルモデルとして配布**したことで、以下の効果が生まれました：

1. **ベースモデルへの依存関係が不可視化**された — アダプタ形式なら必須の `base_model` 指定が不要
2. **671Bパラメータの「独自モデル」に見える** — 実際に変更されたのはMLA射影層のみ
3. **MITライセンス表示を回避しやすい構造**になった — 一見して派生物と分からない

つまり、Rakutenは**LoRAと同じ予算・計算量でファインチューニングしながら、LoRAアダプタで済ませたのではない独自モデルであるかのような印象を対外的に与えることができた**のです。技術的には合理的な選択ですが、この構造が結果的にDeepSeek-V3との関係性の不透明化とMITライセンス表示義務の軽視を容易にした点は、透明性の観点から重大な問題です。
...
配布物にはDeepSeek-V3の重みの**大部分がそのまま含まれており**、これは明確にDeepSeek-V3の派生物です。

ここで重要なのは**配布形態の選択**です。もしRakutenが通常のLoRAアダプタとして成果物を配布していれば、`base_model: deepseek-ai/DeepSeek-V3` という依存関係の明記が必須となり、ライセンス上の関係性は自明でした。しかし、MLAの内蔵低ランク構造をファインチューニングしてフルモデルとして配布する形態を選んだことで、ベースモデルへの依存関係が外見上は不可視化され、MITライセンス表示を「忘れる」ことが容易な状況が生まれました。

結果として、RakutenはDeepSeek-V3との関係性を明示せず、MITライセンス表示も行わず、Apache-2.0のみを表示して全く別のモデルとして公表しています。MITライセンスの要件は最小限（著作権表示とライセンス文の保持）ですが、それすら満たされていない状態です。
...
RakutenはDeepSeek-V3をベースモデルとして使用し、そのMLA射影層を中心にファインチューニングを行いました。実質的にはLoRAと同等の計算コストと手法でありながら、成果物をLoRAアダプタではなくフルモデルとして配布することで、ベースモデルへの依存関係を外見上不可視化しました。その上で、技術的関係性を明示せず、全く別の独自モデルであるかのように「RakutenAI-3.0」として公表しています。

DeepSeek-V3のMITライセンス表示は行われず、Apache-2.0のみが表記されています。MITライセンスが求める最低限の義務（著作権表示の保持）すら満たしていない状態です。
...
### 問題の本質
DeepSeek-V3のMLA射影層をファインチューニングすること自体は、合理的かつ技術的に評価できる手法です。しかしRakutenは、**LoRAと実質同等のコスト・手法でありながら、成果物をLoRAアダプタではなくフルモデルとして配布**することで、ベースモデルとの依存関係を外見上不可視化しました。その上で、DeepSeek-V3との技術的関係性を明示せず、MITライセンス表示も行わないまま、全く別の独自モデルとして「RakutenAI-3.0」を公表しています。

通常のLoRAアダプタ配布であれば、ベースモデルの指定が必須であり、ライセンス関係も自明でした。MLAの内蔵低ランク構造を利用した配布形態の選択が、結果的に透明性とライセンス遵守の両面での問題を生んでいます。公的資金を受けたプロジェクトとして、この不透明さは特に問題です。
