...
**Note**: This investigation demonstrates that RakutenAI-3.0 is a fine-tuned derivative of DeepSeek-V3, with modifications focused on the MLA low-rank projection layers — which are mathematically equivalent to LoRA (low-rank matrix decomposition built into the architecture). The actual computational cost of this fine-tuning is equivalent to LoRA, but by distributing the result as a full model (rather than as a LoRA adapter that would require explicitly declaring `base_model: deepseek-ai/DeepSeek-V3`), the dependency on DeepSeek-V3 was obscured. Rakuten published it as an independent model without displaying the required MIT license or crediting DeepSeek-V3, despite the model weights being 99.94% identical. All findings are reproducible using the provided scripts and data.